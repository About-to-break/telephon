...
class SafeMetricsCollector:
    ...
    def _get_stats_safe(self) -> str:
        ...
        lines = [
            f"# HELP {service_name}_service_time_seconds Service time distribution",
            f"# TYPE {service_name}_service_time_seconds summary",
            f'{service_name}_service_time_seconds{{quantile="0.5",service="{service_name}"}} {p50_service}',
            f'{service_name}_service_time_seconds{{quantile="0.95",service="{service_name}"}} {p95_service}',
            f'{service_name}_service_time_seconds{{quantile="0.99",service="{service_name}"}} {p99_service}',
            f'{service_name}_service_time_seconds_sum{{service="{service_name}"}} {avg_service_time * completed}',
            f'{service_name}_service_time_seconds_count{{service="{service_name}"}} {completed}',
            "",
            f"# HELP {service_name}_wait_time_seconds Queue wait time distribution",
            f"# TYPE {service_name}_wait_time_seconds summary",
            f'{service_name}_wait_time_seconds{{quantile="0.5",service="{service_name}"}} {p50_wait}',
            f'{service_name}_wait_time_seconds{{quantile="0.95",service="{service_name}"}} {p95_wait}',
            f'{service_name}_wait_time_seconds_sum{{service="{service_name}"}} {avg_wait_time * completed}',
